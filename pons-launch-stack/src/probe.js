 ...
 */
export async function discoverCall({ to, candidates, valueEth = "0" }) {
  const report = [];
...
// حالت CLI: node src/probe.js --to 0x.. --sig "buy(uint256,uint256,address)" --args 0,0,0xRECIPIENT --value 0.001
async function main() {
  const a = parseArgs();
  if (!a.to || !a.sig) {
    console.log('مثال: node src/probe.js --to 0xCURVE --sig "buy(uint256,uint256,address)" --args 0,0,0x.. --value 0.001');
    process.exit(1);
  }
  const args = (a.args ?? "").split(",").filter((s) => s.length).map((s) => (/^\d+$/.test(s) ? BigInt(s) : s));
  try {
    const r = await discoverCall({ to: a.to, candidates: [{ sig: a.sig, args }], valueEth: a.value ?? "0" });
